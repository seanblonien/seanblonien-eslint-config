...
import { booleanNameConvention, eslintConfigBase, type EslintConfigBaseOptions } from '@seanblonien/eslint-config-base';
...
export type { EslintConfigBaseOptions } from '@seanblonien/eslint-config-base';

// React/hooks/a11y recommended configs never depend on EslintConfigBaseOptions, so they live
// outside buildConfig — a top-level array isn't a function body, so it isn't subject to
// max-lines-per-function below.
const REACT_RECOMMENDED_CONFIGS: Linter.Config[] = [
...
const REACT_JSX_CONFIG: Linter.Config = {
  files: ['**/*.{tsx,jsx}'],
  plugins: {
...
const buildConfig = (options: EslintConfigBaseOptions): Linter.Config[] => [
  ...eslintConfigBase(options),
  ...REACT_RECOMMENDED_CONFIGS,
  REACT_JSX_CONFIG,
];

/**
 * Sean Blonien's opinionated ESLint flat config for TypeScript/React projects. Extends
 * {@link eslintConfigBase} (see `@seanblonien/eslint-config-base` for the full option set) and
 * adds React, Hooks, and accessibility rules on top.
 *
 * @example
 * ```ts
 * // eslint.config.ts
 ...
 * ```
 */
export const eslintConfigReact = (options: EslintConfigBaseOptions = {}): Linter.Config[] =>
  buildConfig(options);
 ...
 * ```
 */
export const configWithNext = async (
  options: EslintConfigBaseOptions = {},
